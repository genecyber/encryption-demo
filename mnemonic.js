...
(function (root, factory) {
    ...
    _Mnemonic.prototype.toHex = function () {
    ...
    _Mnemonic.prototype.toWords = function () {
        var i = 0, l = this.seed.length, n = _Mnemonic.wc, words = [], x, w1, w2, w3;
        for (; i < l; i++) {
            x = this.seed[i];
            w1 = x % n;
            w2 = (((x / n) >> 0) + w1 ) % n;
            w3 = (((((x / n) >> 0) / n ) >> 0) + w2 ) % n;
            words.push(_Mnemonic.words[w1]);
            words.push(_Mnemonic.words[w2]);
            words.push(_Mnemonic.words[w3]);
        }
        return words;
    };

    _Mnemonic.fromWords = function (words) {
        var i = 0, n = _Mnemonic.wc,
            l = words.length / 3,
            seed = new Uint32Array(l),
            w1, w2, w3;

        for (; i < l; i++) {
            w1 = _Mnemonic.words.indexOf(words[3 * i]);
            w2 = _Mnemonic.words.indexOf(words[3 * i + 1]);
            w3 = _Mnemonic.words.indexOf(words[3 * i + 2]);
            seed[i] = w1 + n * _Mnemonic._mod(w2 - w1, n) + n * n * _Mnemonic._mod(w3 - w2, n);
        }

        return new _Mnemonic(seed);
    };

    _Mnemonic.fromHex = function (hex) {
